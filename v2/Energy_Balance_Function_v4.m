function [T_max,T,Q_in_kW,Q_solar_loss_kW,Rad_Loss_kW,Conv_Loss_kW,HTF_Gain_kW,T_Nodal] = Energy_Balance_Function_v4(N_el_domain,epsilon_sol,epsilon_therm,rho_sol,rho_therm,F_hat_sol,F_hat_therm,T_HTF_in,T_HTF_out,T_infinity,A_Aperature,A_Panel,A_Roof,Receiver_Height,A_Top_Lip,A_Bottom_Lip,UA_HTF,h_Type,h_bar_conv,Q_in,panel_m,Top_Lip,Bottom_Lip)
    ...
    for i=1:4
        for j=1:panel_m 
            T_avg_HTF(j+(i-1)*panel_m)=T_avg_HTF_nodal(j);
        end
    end
    ...
    A(3+panel_m*4)=A_Aperature;
    
    if Top_Lip > 0
        A(3+4*panel_m)=A_Top_Lip;
        A(4+4*panel_m)=A_Aperature;
    end
    if Bottom_Lip > 0
        A(4+4*panel_m)=A_Bottom_Lip;
        A(5+4*panel_m)=A_Aperature;
    end
    ...
    UA_HTF((1+panel_m*4):N_el_domain)=0;
    clear T
    T=T_max;
    ...
    if panel_m > 1
        ...
        for i=1:4
            for j=1:panel_m
               T(i)=T(i)+T_Nodal(j+((i-1)*panel_m))/panel_m;
            end
        end
        ...
        T(7)=T_infinity;
        if Top_Lip > 0
            T(7)=T_Nodal(3+panel_m*4);
            T(8)=T_infinity;
        end
        if Bottom_Lip > 0
            T(8)=T_Nodal(4+panel_m*4);
            T(9)=T_infinity;
        end
    end
