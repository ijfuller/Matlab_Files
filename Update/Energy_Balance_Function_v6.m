function [T_max,T,Q_in_kW,Q_solar_loss_kW,Rad_Loss_kW,Conv_Loss_kW,HTF_Gain_kW,T_Nodal,h_bar_conv,U] = Energy_Balance_Function_v5(N_el_domain,epsilon_sol,epsilon_therm,rho_sol,rho_therm,F_hat_sol,F_hat_therm,T_HTF_in,T_HTF_out,T_infinity,A_Aperature,A_Panel,A_Roof,Receiver_Height,A_Top_Lip,A_Bottom_Lip,UA_HTF,h_Type,h_bar_conv,Q_in,panel_n,panel_m,cover_n,cover_m,Top_Lip,Bottom_Lip,Q_nodal_perc,UA_Type)
    T_max=0;
    Q_in_kW=0;
    T=0;
    Q_solar_loss_kW=0;
    ...
    m_dot_tol=100; %Intialized m_dot tolerance to start loop
    eta=0.9; %Guess effeciency
    while m_dot_tol>0.001 %Tolerance loop for m_dot
        if UA_Type==1
            [U,m_dot] = m_dot_function(eta,Q_in,T_HTF_in,T_HTF_out,Receiver_Height,A_Panel);
            UA_HTF=U;
        end
        A_EB=zeros((N_el_domain-1),(N_el_domain-1));
        b_EB=zeros((N_el_domain-1),1);
        h_bar_conv=ones((N_el_domain-1),1)*h_bar_conv(1);
        ...
        eta=(HTF_Gain_kW/Q_in_kW);
        if UA_Type==1
            m_dot_old=m_dot;
            [U,m_dot] = m_dot_function(eta,Q_in,T_HTF_in,T_HTF_out,Receiver_Height,A_Panel);
            m_dot_tol=abs(m_dot-m_dot_old)/m_dot_old;
        else
            m_dot_tol=0;
        end
    end
