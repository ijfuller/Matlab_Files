function [F_Panel_1_Floor,F_Panel_2_Floor,F_Aper_Floor,F_Aper_Roof,F_Top_Lip_Floor,F_Bottom_Lip_Floor,F_Bottom_Lip_Roof,F_Top_Lip_Roof] = MC_Panel_to_Floor_Function(Receiver_Height,Receiver_Width,N_rays,Top_Lip,Bottom_Lip,El_Nod_Conn_Panel,Global_Nodal_Coord_Panel,El_Nod_Conn_Cover,Global_Nodal_Coor_Cover)
    ...
    for panel=1:set %Loop over number of times MC needs to be run
        ...
        for ii=1:h %Loop over number of elements/panel
            ...
            elseif panel==3 || panel==4 || panel==5 || panel==6 || panel==7 ||panel==8;
                ...
                floor_z_mod=(floor_z-((Receiver_Width/2)-((Receiver_Width/2)*cosd(45))));
                floor_x_mod=(floor_x-((Receiver_Width/2)*cosd(45)));
                x_temp=floor_z_mod*cosd(-67.5)+floor_x_mod*sind(-67.5);
                y_temp=(-1*floor_z_mod).*sind(-67.5)+floor_x_mod.*cosd(-67.5);
                floor_z_mod=x_temp;
                floor_x_mod=y_temp;
            end
            ...
            if plot_MC_Ray==1 && panel==plot_panel;
                ...
                plot3([floor_x_mod(4) floor_x_mod(1)],[floor_y(4) floor_y(1)],[floor_z_mod(4) floor_z_mod(1)],'b','Linewidth',2) 
            end
                ...
                while ray_counter<N_rays %Run loop until number of rays is reached
                    ...
                    if panel==1;
                        [Pass_Fail_Set] = PIIQ_22_5_Function(m,y_int,x_random,y_random,x,y,El_Nod_Conn_Cover,Global_Nodal_Coor_Cover,jj);
                    elseif panel==3 || panel==4 || panel==5 ||panel==6 || panel==7 || panel==8
                        if ii==1
                            [Pass_Fail_Set] = PIIQ_90_Function(m,y_int,x_random,y_random,x,y,El_Nod_Conn_Cover,Global_Nodal_Coor_Cover,jj);
                        end
                    else
                        [Pass_Fail_Set] = PIIQ_67_5_Function(m,y_int,x_random,y_random,x,y,El_Nod_Conn_Cover,Global_Nodal_Coor_Cover,jj);
                    end
            ...
            F=(hit_counter/ray_counter);
                    if panel==1;
                        F_Panel_1_Floor(ii,jj)=F;
                    elseif panel==3
                        F_Aper_Floor(ii,jj)=F;
                    elseif panel==4
                        F_Aper_Roof(ii,jj)=F;
                    elseif panel==5
                        F_Top_Lip_Floor(ii,jj)=F;
                    elseif panel==6
                        F_Top_Lip_Roof(ii,jj)=F;
                    elseif panel==7
                        F_Bottom_Lip_Floor(ii,jj)=F;     
                    elseif panel==8
                        F_Bottom_Lip_Roof(ii,jj)=F;            
                    else panel==2;
                        F_Panel_2_Floor(ii,jj)=F;
                    end
            end
        end
    end
    display('-----------------------------------------------')
    display('Monte Carlo Panel-Floor Completed')
